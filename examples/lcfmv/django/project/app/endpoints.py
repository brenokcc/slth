...
class EditarUsuario(endpoints.EditEndpoint[User]):
    ...
    def get(self):
        initial = {}
        for name in ["adm", "opl", "opa"]:
            initial[name] = Role.objects.filter(
                username=self.instance.username, name=name
            ).exists()
        return (
            super().get().fieldset("Papéis", ("adm", "opl", "opa")).initial(**initial)
        )
...
class AcessoRapido(endpoints.Endpoint):
    def get(self):
        ...
        if self.check_role("adm", "opl"):
            boxes.append(
                icon="list", label="Tipos de Legislação", url="/api/tiposlegislacao/"
            )
        if self.check_role("adm"):
            boxes.append(icon="building", label="Conselhos", url="/api/conselhos/")
            boxes.append(
                icon="person-chalkboard", label="Relatores", url="/api/relatores/"
            )
        if self.check_role("adm", "opl"):
            boxes.append(
                icon="file-contract", label="Legislações", url="/api/legislacoes/"
            )
...
class Consultar(endpoints.PublicEndpoint):
    ...
    tipo_legislacao = forms.ModelChoiceField(
        queryset=TipoLegislacao.objects,
        label="Tipo de Legislação",
        required=False,
        pick=True,
        help_text="Não informar caso deseje buscar em todos os tipos.",
    )
    orgao_julgador = forms.ChoiceField(
        label="Órgão Julgador",
        choices=Acordao.ORGAO_JULGADOR_CHOICES,
        required=False,
        pick=True,
        help_text="Não informar caso deseje buscar em todos os órgãos.",
    )
    natureza = forms.ChoiceField(
        label="Natureza",
        choices=Acordao.NATUREZA_CHOICES,
        required=False,
        pick=True,
        help_text="Não informar caso deseje buscar em todos as naturezas.",
    )
    relator = forms.ModelChoiceField(
        queryset=Relator.objects,
        label="Relator",
        required=False,
        help_text="Não informar caso deseje buscar por todos os relatores.",
    )
    palavas_chaves = forms.CharField(label="Palavras-chaves", required=True)
    criterio_busca = forms.ChoiceField(
        label="Critério da Busca",
        choices=[
    ...
    def get(self):
        return (
            ...
            .info(
                "Busque pelas legislações/acordãos cadastradros no sistema através de uma ou mais palavras-chaves."
            )
            .fields(
                "tipo_documento",
                "tipo_legislacao",
                ...
                "palavas_chaves",
                "criterio_busca",
                "escopo_busca_legislacao",
                "escopo_busca_acordao",
                "horarios",
            )
    ...
    def post(self):
        print(
            type(self.cleaned_data.get("horarios")), self.cleaned_data.get("horarios")
        )
        ...
        if criterio_busca == "toda":
            for campo in escopo_busca:
                resultado = resultado | qs.filter(
                    **{f"{campo}__icontains": palavas_chaves}
                )
        ...
        return (
            resultado.fields("get_descricao", "ementa", "get_arquivo")
            .contextualize(self.request)
            .rows()
            .limit(10)
        )
    ...
    def on_tipo_documento_change(self, controller, values):
        if values.get("tipo_documento") == "legislacao":
            ...
            controller.hide(
                "escopo_busca_acordao", "orgao_julgador", "natureza", "relator"
            )
        else:
...
class RevogarLegislacao(endpoints.Endpoint):
    ...
    def get_revogada_por_queryset(self, queryset):
        return queryset.filter(
            tipo=self.instance.tipo, data__gte=self.instance.data
