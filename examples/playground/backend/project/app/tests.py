...
class ApiTestCase(ServerTestCase):
    ...
    def test_form(self):
        ...
        self.assert_model_count('app.telefone', 2)
        pessoa = self.objects('app.pessoa').last()
    ...
    def test_json(self):
        ...
        self.assert_model_count('app.pessoa', 1)
        self.assert_model_count('app.telefone', 2)
        pessoa = self.objects('app.pessoa').last()
    ...
    def test_serialization(self):
        juca = self.objects('app.pessoa').create(nome='Juca da Silva')
        cidade = self.objects('app.cidade').create(nome='Natal', prefeito=juca)
    ...
    def test_serializer(self):
        telefone = self.objects('app.telefone').create(ddd=84, numero='99999-9999')
        juca = self.objects('app.pessoa').create(nome='Juca da Silva', telefone_pessoal=telefone, data_nascimento=date.today())
        for numero in ('11111-1111', '22222-2222'):
            juca.telefones_profissionais.add(self.objects('app.telefone').create(ddd=84, numero=numero))
        querystrings = {
            None: 'object',
    ...
    def test_actions(self):
        telefone = self.objects('app.telefone').create(ddd=84, numero='99999-9999')
        juca = self.objects('app.pessoa').create(nome='Juca da Silva', telefone_pessoal=telefone, data_nascimento=date.today())
        maria = self.objects('app.pessoa').create(nome='Maria da Silva', data_nascimento=date.today())
        ...
        maria = self.objects('app.pessoa').get(pk=maria.pk)
        data = Serializer(self.objects('app.pessoa').get(pk=maria.pk)).serialize(self.debug)
    ...
    def test_model(self):
        ...
        telefone = self.objects('app.telefone').create(ddd=84, numero='99999-9999')
        juca = self.objects('app.pessoa').create(nome='Juca da Silva', telefone_pessoal=telefone, data_nascimento=today)
        maria = self.objects('app.pessoa').create(nome='Maria da Silva', data_nascimento=next_month)
        qs1 = self.objects('app.pessoa').com_telefone_pessoal()
        qs2 = self.objects('app.pessoa').sem_telefone_pessoal()
        self.assertEqual(qs1.count(), 1)
        self.assertEqual(qs2.count(), 1)

        # using a custom serializer in the queryset
        self.objects('app.pessoa').serializer(
    ...
    def test_roles(self):
        a = self.objects('app.funcionario').create(email='adm@mail.com', is_admin=True)
        ...
        a.save()
        self.assert_model_count('slth.role', 1)
        s1 = self.objects('app.funcionario').create(email='s1@mail.com')
        s2 = self.objects('app.funcionario').create(email='s2@mail.com')
        r1 = self.objects('app.rede').create(nome='r1', supervisor=s1)
        r2 = self.objects('app.rede').create(nome='r2', supervisor=s2)
        
        if self.debug: self.objects('slth.role').debug()
        self.assert_model_count('slth.role', 3)
        redes = self.objects('app.rede').all()
        ...
        g1 = self.objects('app.funcionario').create(email='s1@mail.com')
        g2 = self.objects('app.funcionario').create(email='s2@mail.com')
        va1 = self.objects('app.funcionario').create(email='va1@mail.com')
        vb1 = self.objects('app.funcionario').create(email='vb1@mail.com')
        va2 = self.objects('app.funcionario').create(email='va2@mail.com')
        vb2 = self.objects('app.funcionario').create(email='vb2@mail.com')
        la1 = self.objects('app.loja').create(rede=r1, nome='la1', gerente=g1)
        la1.vendedores.add(va1)
        self.objects('app.produto').create(nome='p1a', loja=la1)
        self.objects('app.produto').create(nome='p2a', loja=la1)
        la2 = self.objects('app.loja').create(rede=r1, nome='la2', gerente=g2)
        la2.vendedores.add(va2)
        self.objects('app.produto').create(nome='p1b', loja=la2)
        self.objects('app.produto').create(nome='p2b', loja=la2)
        if self.debug: self.objects('slth.role').debug()
